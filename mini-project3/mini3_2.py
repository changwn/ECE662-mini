...
x1.shape
x1_label = np.ones((1000),dtype=int)
...
x2.shape
x2_label = np.zeros((1000),dtype=int)
...
plt.subplot(221)
plt.scatter(x[:, 0], x[:, 1], c=label_true, alpha=0.2)
plt.title("True Label")


plt.subplot(222)
y_pred = KMeans(n_clusters=2, random_state=0).fit(x)
plt.scatter(x[:, 0], x[:, 1], c=y_pred.labels_,  alpha=0.2)
...
plt.scatter(x[:, 0], x[:, 1], c=y_pred3.labels_,  alpha=0.2)
